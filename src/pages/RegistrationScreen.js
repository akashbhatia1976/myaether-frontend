  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      Alert.alert('Success', response.message, [
        { text: 'OK', onPress: () => navigation.replace('DashboardScreen', { userId: trimmedUserId }) }
      ]);
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <>
          <Button title="Register" onPress={handleRegister} disabled={!userId.trim()} />
          <Button title="Already have an account? Login" onPress={() => navigation.replace('LoginScreen')} />
        </>
      )}
    </View>
  );
